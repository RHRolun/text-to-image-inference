    ...
    def __call__(self, sample, timestep, encoder_hidden_states):
        ...
        payload = {
            "inputs": [
                ...
                {
                    "name": "timestep", 
                    "shape": np.shape(timestep), 
                    "datatype": "FP32",
                    "data": timestep
                },
                {
                    "name": "encoder_hidden_states", 
                    "shape": np.shape(encoder_hidden_states), 
                    "datatype": "FP32",
...
class OVStableDiffusionPipeline(DiffusionPipeline):
    def __init__(
        ...
        self.height = 64 * 8 # self.unet.input(0).shape[2]
        self.width = 64 * 8 # self.unet.input(0).shape[3]
    ...
    def __call__(
        ...
        **kwargs,
        ...
        for i, t in enumerate(self.progress_bar(timesteps)):
            ...
            noise_pred = self.unet(sample=latent_model_input, timestep=t, encoder_hidden_states=text_embeddings)[self._unet_output]
            # perform guidance
            if do_classifier_free_guidance:
